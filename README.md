...
- axe scans the enabled page, opened hero disclosures, and completed scenario states. Actual script-disabled contexts are tested separately because axe itself requires JavaScript. WebKit link-tab behavior respects the operating system’s keyboard-access preferences.
...
- [src/lib/perspectives.ts](src/lib/perspectives.ts): first-person hero insights and the AI-native-team operating principles. Personal examples must be owner-approved.
- [src/lib/isometric.ts](src/lib/isometric.ts): one 3D isometric basis and shared architectural geometry for the home illustration.
...
The hero’s + markers open native HTML popovers: keyboard/touch activation, Escape/light dismissal, and explicit close controls. They require no client JavaScript. Explicit `tabindex="0"` on opener/close buttons is intentional: it preserves native pointer focus and focus return in Safari. Older browsers without popover support show the insights as ordinary text instead of dead controls.
...
Official Craft Agents and Yabune marks are self-hosted for descriptive identification. Their provenance, preserved proportions, and trademark boundaries are documented in [brand assets](docs/brand-assets.md). They are not original artwork or implied endorsements.
